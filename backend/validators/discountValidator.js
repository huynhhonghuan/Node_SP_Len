...
const validateDiscountData = (isUpdate = false) => {
    return [
        check('code')
            ...
            .custom(async (value, { req }) => {
                // Nếu là cập nhật, kiểm tra xem mã giảm giá có trùng với bản ghi hiện tại không
                if (isUpdate) {
                    const discountId = req.params.id; // Lấy ID của bản ghi từ params
                    const existingDiscount = await mongoose.model('Discount').findOne({ code: value, _id: { $ne: discountId } });
                    if (existingDiscount) {
        ...
        check('isActive')
            .isBoolean().withMessage('Trạng thái không được rỗng!'),
